...
class ChatReducer : Reducer<ChatStore.State, ChatStateChanges> {
    override fun ChatStore.State.reduce(result: ChatStateChanges): ChatStore.State {
        return when (result) {
            is ChatStateChanges.ListChanged -> copy(pagingData = result.messageListData)
            ...
            is ChatStateChanges.CurrentlyEditingTextChanged -> copy(currentlyEditingMessage = result.message)
        }
    }
}